...
Location hints are suggestions, not guarantees. Refer to [Data location](/durable-objects/how-to/configure-data-location/) for available regions and details.
...
Refer to [Access Durable Objects storage](/durable-objects/how-to/access-storage/) for more details on the SQL API.
...
For more details, see [Durable Objects: Easy, Fast, Correct — Choose three](https://blog.cloudflare.com/durable-objects-easy-fast-correct-choose-three/) and the [glossary](/durable-objects/platform/glossary/).
...
Refer to [Invoke methods](/durable-objects/how-to/invoke-methods/) for more details on RPC and the legacy `fetch()` handler.
...
Refer to [Error handling](/durable-objects/how-to/handle-errors/) for details on error properties, retry strategies, and exponential backoff patterns.
...
- The [WebSocket Hibernation API](/durable-objects/how-to/use-websockets/#durable-objects-hibernation-websocket-api) exposes `webSocketError`, `webSocketMessage`, and `webSocketClose` handlers for their respective WebSocket events.
...
Refer to [Use WebSockets](/durable-objects/how-to/use-websockets/) for more details.
...
Refer to [Durable Objects migrations](/durable-objects/how-to/configure-migrations/) for more details on class migrations, and [Testing with Durable Objects](/durable-objects/how-to/test-durable-objects/) for comprehensive testing patterns including SQLite queries and alarm testing.
